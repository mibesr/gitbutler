...
fn validate_name(
    ...
    if ctx.repository().find_reference(&reference.name).is_ok() {
        return Err(anyhow!(
    ...
    if let Some(remote_name) = default_target.push_remote_name {
        let refname = reference.remote_reference(remote_name)?;
        if ctx.repository().find_reference(&refname).is_ok() {
            return Err(anyhow!(
                "A git reference with the name {} exists",
                &reference.name
            ));
    ...
    // assert that there are existing patch reference with this name
    if state
        .list_all_branches()?
        .iter()
